...
import { ScriptType, FeeRate, TransactionService } from "../transaction.service";
import { MintNftParams, MintParams, NftScript, NftSdk, ReadNftResponse } from "./nft.types";
...
export class NftService {
    ...
    async createNftFactory(mintData: MintNftParams, privateKey: string): Promise<string> {
        await this.initialize();
        const resp = await this.nftSdk.factoryCell.mint(mintData);
        this.logger.info(resp.rawTransaction);
        this.logger.info(resp.typeScript);
        this.logger.info(resp.usedCapacity);

        const signedTx = this.nftSdk.ckb.signTransaction(privateKey)(resp.rawTransaction);
        const txHash = await this.nftSdk.ckb.rpc.sendTransaction(signedTx, "passthrough");
        this.logger.info(txHash);
        return txHash;
    }

    async createNft(mintData: MintParams, privateKey: string): Promise<string> {
        await this.initialize();
        const resp = await this.nftSdk.nftCell.mint(mintData);
        this.logger.info(resp.rawTransaction);
        this.logger.info(resp.nftTypeScript);
        this.logger.info(resp.usedCapacity);
